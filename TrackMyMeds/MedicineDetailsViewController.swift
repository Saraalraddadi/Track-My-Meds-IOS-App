...
class MedicineDetailsViewController: UIViewController, UITextFieldDelegate {
    ...
    @IBOutlet weak var takeImgView: UIImageView!
    
    var medi: Medicine?
    var selectedMedImg = UIImage()
    ...
    override func viewDidLoad() {
        ...
        if medi != nil {
            ...
            if medi?.medImgData != nil {
                
                    self.takeImgView.image = selectedMedImg 
                
                
            }else {
                let screenSize: CGRect = UIScreen.main.bounds
                let frame = CGRect(x: (screenSize.width/2) - (mediShapeImage.frame.width/2), y: mediShapeImage.frame.origin.y, width: mediShapeImage.frame.width, height: mediShapeImage.frame.height)
                takeImgView.isHidden = true
                mediShapeImage.frame = frame
            }
        }
